...
from polls.viewsets.magiclink import migrate_device_votes_to_user  # shared helper
...
class CookieSocialLoginView(SocialLoginView):
    ...
    then issues JWT and sets them as HttpOnly cookies (same as password/magic-link).
    """
    client_class = OAuth2Client
    callback_url = None  # override in subclass

    def post(self, request, *args, **kwargs):
        # Let dj-rest-auth/allauth perform the OAuth exchange first.
        ...
        # Optional: migrate device votes to the authenticated user
...
class GoogleCookieLogin(CookieSocialLoginView):
    ...
    callback_url = "http://localhost:3000/auth/google/callback"  # keep in sync with FE
